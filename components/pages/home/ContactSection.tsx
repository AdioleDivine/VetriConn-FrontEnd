...
export const ContactSection = ({ id }: ContactSectionProps) => {
  const [formData, setFormData] = useState<ContactMessage>({ full_name: "", email: "", description: "" });
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    else if (name === "message") setFormData((prev) => ({ ...prev, description: value }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.full_name.trim() || !formData.email.trim() || !formData.description.trim()) {
    ...
    try {
      ...
      showToast({ type: "success", title: "Message Sent!", description: "Thank you for contacting us. We'll get back to you soon." });
      setFormData({ full_name: "", email: "", description: "" });
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : "Failed to send message. Please try again.";
      setSubmitStatus({ type: "error", message: errorMessage });
      showToast({ type: "error", title: "Message Failed to Send", description: errorMessage });
    } finally {
