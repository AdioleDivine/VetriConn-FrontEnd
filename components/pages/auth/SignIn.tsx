...
export const SignIn = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setErrors({});
    try {
      signInSchema.parse({ email, password, terms: true });
  ...
  return (
    <div className="flex min-h-screen font-open-sans">
      ...
      {/* Form side - Desktop */}
      <div className="flex-1 items-center justify-center p-16 bg-white relative hidden md:flex">
        ...
        <div className="w-full max-w-[500px]">
          ...
          <form onSubmit={handleSubmit}>
            <label htmlFor="email-desktop" className="block text-sm text-text-muted mb-1 font-medium">Email</label>
            <input type="email" id="email-desktop" value={email} onChange={(e) => setEmail(e.target.value)} className={clsx("block w-full py-3 px-3 mb-4 border border-gray-300 rounded-[10px] text-base outline-none focus:border-primary", errors.email && "border-primary", email.trim() && !errors.email && "border-primary")} />
            {errors.email && <span className="text-primary text-sm -mt-3 mb-4 block">{errors.email}</span>}
            <label htmlFor="password-desktop" className="block text-sm text-text-muted mb-1 font-medium">Password</label>
            <input type="password" id="password-desktop" value={password} onChange={(e) => setPassword(e.target.value)} className={clsx("block w-full py-3 px-3 mb-4 border border-gray-300 rounded-[10px] text-base outline-none focus:border-primary", errors.password && "border-primary", password.trim() && !errors.password && "border-primary")} />
            ...
            <div className="flex items-start gap-2 text-sm mb-6">
              ...
              <label htmlFor="terms-desktop">I agree to the <a href="#" className="text-primary no-underline">Terms of Service</a> and <a href="#" className="text-primary no-underline">Privacy Policy</a></label>
            </div>
      ...
      <div className="p-6 w-full box-border md:hidden">
        <a href="/signup" className="absolute top-9 right-4 left-auto bg-primary text-white border-none rounded-[20px] py-2.5 px-5 text-sm font-medium cursor-pointer z-10 transition-colors hover:bg-primary-hover no-underline">Sign Up</a>
        ...
        <div className="flex flex-col gap-6">
          ...
          <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
            ...
            <button type="submit" className="bg-primary text-white border-none rounded-lg py-4 px-8 text-base font-semibold cursor-pointer w-full mt-12 transition-colors hover:enabled:bg-primary-hover disabled:bg-gray-200 disabled:cursor-not-allowed disabled:text-gray-400" disabled={isButtonDisabled}>
