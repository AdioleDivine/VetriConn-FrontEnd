...
export function useUserProfile() {
  ...
  const userProfile = data?.data?.user ? {
    name: `${data.data.user.first_name || ''} ${data.data.user.last_name || ''}`.trim() || "User",
    ...
    avatar: data.data.user.picture && data.data.user.picture.trim() && data.data.user.picture.trim() !== "/images/richmond.svg" 
